...
func (nbdp *NetworkBridgeDevicePlugin) Allocate(ctx context.Context, r *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, req := range r.ContainerRequests {
		...
		if err != nil {
			glog.V(3).Info(fmt.Printf("Failed to marshal network interface description due to: %s", err.Error()))
			continue
		}
		...
		go func(assignment *Assignment) {
			...
			for i := 1; i <= attachmentRetries; i++ {
				if attached := func() bool {
					...
					if err != nil {
						glog.V(3).Info(fmt.Printf("Failed to obtain container's pid, due to: %s", err.Error()))
						return false
					}
